...
(ถอดความไว้ที่ `horizon-flow-transcript-th.md`) เป็นกลยุทธ์สองทาง 3 ตัว

- `horizon_flow_v4` — ตามคลิปตรงตัว: EMA100 + ต้องไล่ high/low ใหม่ 3 แท่งจึงเปลี่ยนฝั่ง +
...
- `horizon_flow_v4_trail` — ต่างที่จุดออกเท่านั้น: ไม่มี TP, เลื่อน SL มาที่ทุนเมื่อ +1R, trail 2 ATR, ออกเมื่อกลับฝั่ง
...
ไว้ใน `lib/indicators-v2.ts` แล้วแปลงเป็น **40 กลยุทธ์** (อินดิเคเตอร์ละ 2 โหมด) รวมกับของเดิมเป็น 53 กลยุทธ์
