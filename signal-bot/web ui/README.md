...
(ถอดความไว้ที่ `horizon-flow-transcript-th.md`) เดิมเป็นกลยุทธ์สองทาง 3 ตัว
**ตอนนี้เหลือ `horizon_flow_v4_strict` ตัวเดียวในรายการ** — ตัวตามคลิปและ trail ถูกถอดออกตามคำขอของผู้ใช้
(ผลบน 1h −67% และ −72%) กฎของทั้งสองยังเรียกผ่าน `horizonFlowV4()` ตรงได้ และเทสต์ยังตรวจกลไกของทั้งสามแบบ

- ~~`horizon_flow_v4`~~ (ถอดออก) — ตามคลิปตรงตัว: EMA100 + ต้องไล่ high/low ใหม่ 3 แท่งจึงเปลี่ยนฝั่ง +
  Stochastic อยู่ในโซน (เข้าได้จนกว่า %K ข้าม 50) + แท่งกลืนกิน/pin bar · SL 1.5 ATR TP 2R
- `horizon_flow_v4_strict` — ต่างที่จุดเข้าเท่านั้น: ต้องเห็น %K ตัด %D, EMA ต้องชัน, SL ≥ 5 เท่าของต้นทุน
- ~~`horizon_flow_v4_trail`~~ (ถอดออก) — ต่างที่จุดออกเท่านั้น: ไม่มี TP, เลื่อน SL มาที่ทุนเมื่อ +1R, trail 2 ATR, ออกเมื่อกลับฝั่ง

ทั้งสามใช้ทะเบียน `V3_REGISTRY` และตัวจำลอง `simulateExposure` ร่วมกับ v3 ต่างกันแค่เลขเวอร์ชันใน UI
**ปิดไม้ที่ราคา SL/TP จริง** ผ่านช่อง `exitFill` (ตรวจด้วย high/low, แตะทั้งคู่ในแท่งเดียวนับ SL ก่อน,
เปิดแท่งกระโดดข้ามระดับปิดที่ราคาเปิด) กลยุทธ์อื่นไม่มีช่องนี้จึงได้ผลเหมือนเดิมทุกตัวเลข

**ผลวัด (ค่าตั้งต้น ไม่ได้ปรับค่าใด ๆ)** — `npx tsx "signal-bot/web ui/research-v4/horizon-flow-eval.ts"`
ต้นทุน futures taker 0.05% + slippage 0.03% ต่อขา + funding 0.01%/8 ชม. · train/test แบ่งที่ 17 พ.ค. 2026

| BTCUSDT | ตามคลิป (train / test) | กรองจุดเข้า | trailing exit |
|---|---|---|---|
| 5m | −90.6% / −72.6% | −50.6% / −26.1% | −92.8% / −76.5% |
| 15m | −60.0% / −23.8% | −35.0% / −4.3% | −60.5% / −30.0% |
| 30m | −30.8% / −20.4% | −15.7% / −16.5% | −28.1% / −21.7% |

- ข้ามเหรียญ ETH/SOL/BNB/XRP ที่ 15m และ 30m: **บวก 0 จาก 8 ช่อง ทุกรหัส**
- ถ้าไม่หักต้นทุนเลย ค่าเฉลี่ยต่อไม้อยู่ราว −0.1R ถึง +0.2R และสลับเครื่องหมายระหว่าง train/test
  แปลว่า **ท่านี้ไม่มี edge ที่วัดได้บน crypto** ค่าธรรมเนียมเป็นแค่ตัวที่ทำให้ขาดทุนชัดขึ้น
- บน 1m ใช้ไม่ได้ในทางคณิตศาสตร์: SL 1.5 ATR ของ BTC 1m ≈ 0.07% เล็กกว่าต้นทุนไป–กลับเอง
- ผล 12 ไม้ในคลิป (win 58%) เป็นทองคำบน MT5 ซึ่งสเปรดต่ำกว่ามาก และน้อยเกินกว่าจะใช้ยืนยันอะไรได้

เก็บไว้ในทะเบียนตามที่ผู้ใช้ขอ เพื่อให้ทดลองปรับค่าบนเว็บและเทียบกับ `orderflow_v3` ได้ — ไม่ใช่เพราะผ่านเกณฑ์
บอท Telegram ใช้รหัสเหล่านี้ได้ แต่จะไม่แสดงบรรทัดวิเคราะห์ (insight ของ v3 อธิบายด้วยแรงซื้อขายสุทธิ)

## OrderFlow V3 — แรงซื้อขายสุทธิ (กลุ่ม เวอร์ชัน 3)

`lib/indicators-v3-OrderFlow.ts` กับกลยุทธ์ 3 ตัวในกลุ่ม **เวอร์ชัน 3**

- `orderflow_v3` — เทรดสองทาง (เปิดสถานะขายได้)
- `orderflow_v3_long` — เฉพาะฝั่งซื้อ ใช้กับบัญชี Spot ได้ ไม่มีต้นทุน funding
- `orderflow_v3_short` — เฉพาะฝั่งขาย ต้องเทรดบน perpetual futures

สัญญาณมาจาก `takerBuyBaseVolume` ซึ่งมีอยู่ในทุกแท่งแต่ไม่มีกลยุทธ์อื่นในโปรเจกต์นี้เคยใช้:
สะสมแรงซื้อขายสุทธิ 5 วัน แล้วลบค่าเฉลี่ยของตัวเอง 120 วัน (ขั้นลบค่าเฉลี่ยขาดไม่ได้ ไม่งั้น
กลยุทธ์จะมีอคติฝั่งขายถาวรและทำกำไรเพราะปีที่ทดสอบเป็นขาลง ไม่ใช่เพราะทำนายถูก)

**ต้องรู้ก่อนใช้**

- ต้องมีข้อมูลย้อนหลังราว **125 วัน** ในช่วงที่เลือก ก่อนกลยุทธ์จะเริ่มให้สัญญาณ
  ช่วงก่อนหน้านั้นเป็นสถานะว่างและบอกไว้ในคอลัมน์เหตุผลว่า "รอสะสม"
- ถือสถานะเฉลี่ยหลายวัน จึงเป็นกลยุทธ์ swing ที่ใช้แท่ง 15m/30m เป็นตัวสุ่มสัญญาณ
- **ต้องกระจายหลายเหรียญ** ผลรายเหรียญไม่สม่ำเสมอและยังไม่มีนัยสำคัญทางสถิติ
- ไม่มี stop loss ตามราคา สัญญาณเป็นตัวกำหนดการออกทั้งหมด

**กลยุทธ์สองทางใช้ตัวจำลองคนละตัว** (`simulateExposure`) ซึ่งรองรับสถานะขาย ขนาดไม้
และต้นทุน funding ตั้งค่า Funding/8 ชม. ได้ในฟอร์ม (ค่าเริ่มต้น 0.01% ตั้ง 0 เพื่อปิด)
กลยุทธ์ Spot ทั้ง V1/V2 ไม่ถูกกระทบ

### โครงไฟล์ของ v3

กลยุทธ์ v3 ทั้งหมดอยู่ใน **`lib/indicators-v3.ts` ไฟล์เดียว** แบ่งเป็นสามส่วน:
ชนิด `V3Result` → ตระกูล OrderFlow → ทะเบียน `V3_REGISTRY` + `V3_PARAM_META`

ตระกูล ShortTrade (SMC) **ไม่ใช่โค้ดของผลิตภัณฑ์แล้ว** เพราะวัดบนข้อมูลเต็มปีแล้ว
ขาดทุนทุก timeframe และพิสูจน์ได้ว่าแก้ด้วยการปรับค่าไม่ได้ จึงย้ายออกจาก `lib/`
ไปเป็นเส้นฐานงานวิจัยที่ `research-v3/shorttrade-baseline.ts` ซึ่งมีแต่สคริปต์
ในโฟลเดอร์นั้นเรียกใช้

`V3Result` มีเฉพาะช่องที่มีค่าจริง 15 ช่อง เวอร์ชันก่อนมีอีก 14 ช่อง
(stop, target, rsi, adx, แนวรับ/ต้าน ฯลฯ) ที่ตกทอดมาจากตระกูล SMC
และเป็น `null` ทุกแท่งตลอดกาล แต่ยังถูกเขียนลงไฟล์ Export เป็นคอลัมน์ว่าง

### เพิ่มกลยุทธ์ v3 ใหม่

แก้ `lib/indicators-v3.ts` ที่เดียว: เขียนฟังก์ชันที่คืน `V3Result`
เพิ่มรหัสใน `V3StrategyId` และเพิ่มหนึ่งรายการใน `V3_REGISTRY`
ให้ครบทุกช่องของ `V3Definition` (ถ้ามีพารามิเตอร์ใหม่ก็เพิ่มป้ายใน `V3_PARAM_META`)
แล้วกลยุทธ์จะไปปรากฏเองในทุกทะเบียน

**ต้องรัน `research-v3/entry-edge.ts` ก่อนลงทะเบียน** ถ้าผลตอบแทนล่วงหน้า
ไม่ต่างจากศูนย์ กลยุทธ์นั้นจะขาดทุนแน่นอนหลังหักต้นทุน

**[อ่านการวิเคราะห์ฉบับเต็มได้ที่ indicators-v3-shorttrade-th.md](../../indicators-v3-shorttrade-th.md)**
รวมข้อค้นพบสำคัญว่า *ความถี่การเทรดกับด่านต้นทุนขัดกันเองบน timeframe ต่ำ* พร้อมตารางที่วัดได้จริง
เช่น ต้นทุนไป–กลับ 0.31% คิดเป็น 6.6 ATR บน 1m แต่เพียง 1.2 ATR บน 15m

```bash
npm run web:test   # รวมเทสต์ v3 17 ข้อ (ผลิตภัณฑ์ 14 + เส้นฐานงานวิจัย 10)
npm run web:ui     # restart เพื่อโหลดกลยุทธ์ใหม่
```

## อินดิเคเตอร์เวอร์ชัน 2 — 20 ตัวจากเอกสาร TradingView

เพิ่มอินดิเคเตอร์ 20 รายการจาก [tradingview-popular-indicators-th.md](../../tradingview-popular-indicators-th.md)
ไว้ใน `lib/indicators-v2.ts` แล้วแปลงเป็น **34 กลยุทธ์** (อินดิเคเตอร์ละ 2 โหมด = 40 ลบโหมดพื้นฐาน 6 ตัวที่ถูกถอดออก)
โหมดพื้นฐานที่ถูกถอดออกตามคำขอของผู้ใช้: `obv_v2` · `vwap_v2` · `ut_bot_v2` · `macd_v2` · `bollinger_v2` · `stoch_rsi_v2`
(โหมด `_filtered` ของทั้งหกตัวยังอยู่) · กลยุทธ์ชุดเดิม (V1) เหลือ 10 ตัว หลังถอด `ut_bot` · `cm_macd` · `squeeze_momentum`

- `<ชื่อ>_v2` — กฎพื้นฐานของอินดิเคเตอร์ตัวนั้นล้วน ๆ ใช้ตอบว่า “ตัวนี้ดีจริงไหม”
- `<ชื่อ>_v2_filtered` — กฎเดิม + ตัวกรอง EMA เทรนด์/ADX-DI/ความผันผวน + ATR stop, trailing และเวลาถือ
  ใช้ตอบว่า “ตัวกรองช่วยหรือทำลายสัญญาณดิบ” เทียบกันได้ตรง ๆ เพราะใช้อินดิเคเตอร์และข้อมูลชุดเดียวกัน

ในรายการ **กลยุทธ์** จะเห็นหัวข้อคั่น เวอร์ชัน 1 / เวอร์ชัน 2 พร้อมป้ายกลุ่มการใช้งาน (แนวโน้ม, โมเมนตัม, ความผันผวน …)
มีปุ่ม **V1** / **V2** เลือกทั้งเวอร์ชัน และช่องค้นหาสำหรับกรองรายการ
ค่าเริ่มต้นเลือกไว้ทุกตัวยกเว้น Lorentzian ซึ่งใช้เวลาคำนวณมากกว่าตัวอื่น

**คำอธิบายกฎสัญญาณของทุกตัว พร้อมเหตุผลที่เลือกกฎแบบนั้น อยู่ใน
[indicators-v2-signals-th.md](../../indicators-v2-signals-th.md)** รวมถึงผลทดสอบเบื้องต้นและข้อจำกัดที่ยังเหลือ

จุดที่ v2 แก้จาก v1: RSI เปลี่ยนจากสภาวะค้างในโซนเป็นจุดตัดขากลับ, MACD แก้เส้น Signal จาก SMA เป็น EMA
ตามสูตรมาตรฐาน, Squeeze Momentum บังคับให้เข้าจากการคลายตัวของ Squeeze จริง (v1 คำนวณไว้แต่ไม่ได้ใช้)
กลยุทธ์เดิมทั้ง 13 ตัวไม่ถูกแก้พฤติกรรม และอินดิเคเตอร์ v2 จะถูกคำนวณเฉพาะเมื่อเลือกกลยุทธ์ v2 เท่านั้น

พารามิเตอร์ของกลยุทธ์ v2 ประกาศป้ายภาษาไทยและขอบเขตไว้ใน `V2_PARAM_META` ที่เดียว
ทั้งช่องกรอกในหน้าเว็บและการตรวจค่าฝั่ง server อ่านจากที่นั่น เพิ่มอินดิเคเตอร์ใหม่จึงแก้แค่ `lib/indicators-v2.ts`
...
หน้าจอแบ่งสองฝั่งถาวร: **แถบตั้งค่าอยู่ซ้าย ผลลัพธ์อยู่ขวา** ปรับค่าแล้วกดรันได้โดยไม่ต้องเลื่อนหน้าขึ้นลง เมื่อหน้าต่างแคบกว่า 1080px แถบตั้งค่าจะย้ายไปอยู่ด้านบนแทน

1. เลือกคู่เหรียญ แล้วติ๊กชิป **ช่วงแท่งเทียน** ที่แถบซ้าย จะเลือกกี่ช่วงก็ได้ หรือกด **ทั้งหมด** / **ล้าง** ปุ่ม ★ บนชิปกำหนด **ช่วงหลัก** คือช่วงที่ใช้วาดกราฟและดูค่ารายแท่ง (ช่วงหลักจะถูกรันเสมอ) เมื่อเลือกแหล่งข้อมูลเป็นไฟล์ในเครื่อง ชิปจะเปิดเฉพาะ timeframe ที่ชุดข้อมูลนั้นมีครบ
...
3. ติ๊กกลยุทธ์ที่ต้องการในรายการ **กลยุทธ์** จะเลือกกี่ตัวก็ได้ หรือกด **ทั้งหมด** / **ล้าง** ระบบคำนวณและแสดงผลเฉพาะกลยุทธ์ที่ติ๊กไว้เท่านั้น (คำอธิบายกลยุทธ์อยู่ใน tooltip และในส่วนปรับพารามิเตอร์) ปุ่ม ★ ท้ายรายการกำหนด **กลยุทธ์หลัก** คือกลยุทธ์ที่ใช้วาดกราฟ ค่ารายแท่ง และตัวอย่างข้อความ Telegram (กลยุทธ์หลักจะถูกรันเสมอ) ปรับพารามิเตอร์รายกลยุทธ์ได้ในส่วน **ปรับพารามิเตอร์** ค่าที่ปรับจะคงอยู่ระหว่างใช้งานหน้านี้
4. เลือกโหมดเปิดแท่งถัดไป / โหมดเดิม / ทั้งสองโหมด พร้อมค่าธรรมเนียมและ slippage
5. กดรัน แถบบนขวาสรุป คู่เหรียญ / ช่วงหลัก / กลยุทธ์หลัก และจำนวนแท่ง ถัดลงมาเป็นแถบตัวเลขหลัก 4 ค่า แล้วแบ่งเป็น 4 แท็บ: **กราฟและสัญญาณ**, **ผลการทดสอบ**, **รายการเทรด**, **ค่ารายแท่ง** แท็บกราฟแสดงกราฟผลตอบแทนสะสมและ **อันดับผลตอบแทน 6 อันดับแรก** ไว้ในหน้าเดียวกัน จึงเทียบผลได้โดยไม่ต้องสลับแท็บ; หมายเหตุข้อมูลพับไว้เป็นบรรทัดเดียวใต้แถบบน และจะกางเองเมื่อมีช่วงแท่งเทียนถูกข้าม
...
- `public/` — HTML/CSS/JavaScript และกราฟ Canvas ไม่มี CDN; ฟอนต์ Sarabun (ข้อความ) และ IBM Plex Mono (ตัวเลข) เก็บไว้ใน `public/fonts` ทั้งคู่เป็น SIL Open Font License
