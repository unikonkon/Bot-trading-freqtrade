...
### ตระกูล ShortTrade (SMC) ถูกถอดออกจากทะเบียนแล้ว

`lib/indicators-v3-ShortTrade.ts` ยังมีฟังก์ชัน `shortTradeV3()` อยู่ แต่ `shorttrade_v3*`
**ไม่ใช่กลยุทธ์ที่เลือกได้ในเว็บอีกแล้ว** เพราะวัดบนข้อมูลเต็มปีแล้วขาดทุนทุก timeframe
และพิสูจน์ได้ว่าแก้ด้วยการปรับค่าไม่ได้ ยังเรียกใช้ได้จากสคริปต์วิจัยเพื่อเทียบเป็นเส้นฐาน

### โครงไฟล์ของ v3

| ไฟล์ | หน้าที่ |
|---|---|
| `lib/indicators-v3-core.ts` | ชนิด `V3Result` · `V3_REGISTRY` · `V3_PARAM_META` — ไม่มีกลยุทธ์อยู่ในตัวเอง |
| `lib/indicators-v3-OrderFlow.ts` | กลยุทธ์ที่ลงทะเบียนอยู่จริง |
| `lib/indicators-v3-ShortTrade.ts` | ตระกูลที่ถูกถอดออกจากทะเบียน เหลือไว้เป็นเส้นฐานงานวิจัย |

แกนกลาง import ตระกูลต่าง ๆ เข้ามาลงทะเบียน ส่วนตระกูลต่าง ๆ import กลับมาเฉพาะชนิด
(`import type`) จึงไม่มีวงจร import ตอน runtime

### เพิ่มกลยุทธ์ v3 ใหม่

เขียนฟังก์ชันที่คืน `V3Result` ไว้ในไฟล์ของตระกูลตัวเอง แล้วแก้ `lib/indicators-v3-core.ts`
ที่เดียว: เพิ่มรหัสใน `V3StrategyId` และเพิ่มหนึ่งรายการใน `V3_REGISTRY`
...
```bash
npm run web:test   # รวมเทสต์ v3 18 ข้อ (แกนกลาง 7 + ShortTrade 11)
